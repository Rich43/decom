...
public class GifHeaderParser {
    ...
    private void readBitmap() {
        ...
        gifFrame.interlace = (i2 & DESCRIPTOR_MASK_INTERLACE_FLAG) != 0;
        if (z2) {
        ...
        while (true) {
            try {
                ...
                if (Log.isLoggable(TAG, 3)) {
                    Log.d(TAG, "Error Reading Block n: " + i3 + " count: " + i4 + " blockSize: " + this.blockSize, e);
                }
                this.header.status = 1;
                return;
            }
    ...
    private void readGraphicControlExt() {
        ...
        int i2 = read();
        GifFrame gifFrame = this.header.currentFrame;
        int i3 = (i2 & GCE_MASK_DISPOSAL_METHOD) >> 2;
        gifFrame.dispose = i3;
        if (i3 == 0) {
            gifFrame.dispose = 1;
        }
